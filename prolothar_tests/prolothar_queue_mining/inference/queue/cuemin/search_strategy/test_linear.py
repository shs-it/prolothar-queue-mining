...
class TestLinear(unittest.TestCase):
    ...
    def test_infer_queue_toy_example(self):
        queue_inference = CueMin(record_candidates=True, search_strategy_name='linear')
        observed_arrivals = [
            (Job('A'), 3),
            (Job('B'), 4),
            (Job('C'), 5),
    ...
    def test_infer_synthetic_lifo_queue_with_one_server(self):
        queue_inference = CueMin(verbose=True, search_strategy_name='linear', patience=3)
