...
class TestWeightedSampling(unittest.TestCase):
    ...
    def test_infer_queue_toy_example(self):
        queue_inference = CueMin(record_candidates=True, search_strategy_name='weighted_sampling', patience=3)
        observed_arrivals = [
            (Job('A'), 3),
            (Job('B'), 4),
            (Job('C'), 5),
    ...
    def test_infer_synthetic_fifo_queue_with_one_server(self):
        queue_inference = CueMin(search_strategy_name='weighted_sampling', patience=3)
    ...
    def test_infer_synthetic_fifo_queue_with_one_server_poisson_service_time(self):
        queue_inference = CueMin(search_strategy_name='weighted_sampling', patience=10)
    ...
    def test_infer_synthetic_fifo_queue_with_one_server_with_batching(self):
        ...
        self.assertEqual('ServiceTime(DegenerateDistribution(5))', discovered_model.get_service_time_name())
        self.assertAlmostEqual(3, discovered_batch_size_distribution.get_mean(), delta=0.2)

    def test_infer_synthetic_lifo_queue_with_one_server(self):
        queue_inference = CueMin(verbose=True, search_strategy_name='weighted_sampling', patience=10)
    ...
    def test_infer_synthetic_pqc_queue_with_one_server(self):
        queue_inference = CueMin(
            ...
            search_strategy_name='weighted_sampling',
            patience=10)
