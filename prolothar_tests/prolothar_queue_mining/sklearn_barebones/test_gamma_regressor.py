...
from prolothar_queue_mining.sklearn_barebones.linear_model import BarebonesGammaRegressor

class TestBarebonesGammaRegressor(unittest.TestCase):

    def test_fit_and_predict(self):
        X, y_train = load_diabetes(return_X_y=True)
        model = BarebonesGammaRegressor()
        model.fit(X, y_train)
